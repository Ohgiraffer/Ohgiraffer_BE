...
public record UpdateSubmissionBoxRequest(
        ...
        List<
                @NotNull(message = "제출 항목에는 null을 입력할 수 없습니다.")
                        UpdateSubmissionBoxItemRequest
                > items
) {

    public UpdateSubmissionBoxCommand toCommand(
            Long submissionBoxId
    ) {
        List<UpdateSubmissionBoxItemCommand> itemCommands =
        ...
        return new UpdateSubmissionBoxCommand(
                submissionBoxId,
                projectName,
                targetScope,
                startAt,
