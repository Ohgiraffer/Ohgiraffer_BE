...
public record ChatChannelDetailResponse(
    ...
    public record MemberResponse(Long userId, String memberName, String email, String role, Instant joinedAt, boolean isRead) {
    }
    ...
    public static ChatChannelDetailResponse from(ChatChannelDetailResult result) {
        List<MemberResponse> members = result.members().stream()
                .map(m -> new MemberResponse(m.userId(), m.memberName(), m.email(), m.role(), m.joinedAt(), m.isRead()))
                .toList();
